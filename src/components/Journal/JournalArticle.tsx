...
export default function JournalArticle({ post }: JournalArticleProps) {
  return (
    <article className="rounded-[24px] bg-[#D2D2D2B2] px-[36px] pb-[20px] pt-[32px] drop-shadow-md">
      <header className="mb-[13px]">
        <div className="mb-[7px] flex flex-row gap-x-[18px]">
          <p className="text-[7.5px] leading-tight text-[#989898]">{post.series}</p>
        ...
        <h2 className="text-[13.4px] leading-tight text-[#373737]">{post.title}</h2>
      </header>

      {post.excerpt && (
        <p className="mb-[14px] text-[11px] leading-tight text-[#989898]">{post.excerpt}</p>
      )}

      {post.body && (
        <div className="text-[11px] leading-tight text-[#373737]">
          <PortableText
            value={post.body}
            components={{
              block: {
                ...
                image: ({ value }: any) => {
                  ...
                  return (
                    <div className="mb-[14px]">
                      ...
                      {value.caption && (
                        <p className="mt-[4px] text-left text-[7.5px] leading-tight text-[#989898]">
                          {value.caption}
                        </p>
                      )}
                    </div>
